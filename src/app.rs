...
/// Hosted sample datasets (synthetic, generated by testdata/regenerate.sh,
/// published as assets of the `samples` GitHub release).
const SAMPLE_BASE_URL: &str =
    "https://github.com/gsueur/geopq-workbench/releases/download/samples";
const SAMPLE_DATASETS: &[(&str, &str, &str)] = &[
    (
        "1M points, raw export (22 MB)",
        "One million random points over France and its neighbors, written \
         the way a plain database export comes out: no spatial order, no \
         bbox index. Open the file info to see the quality scorecard, \
         then let Optimize fix it.",
        "points_1m_wgs84.parquet",
    ),
    (
        "20k lines (1 MB)",
        "Random 3-vertex polylines around Paris, raw export.",
        "lines_20k_wgs84.parquet",
    ),
    (
        "5k polygons, EPSG:2154 (2 MB)",
        "Buffered random polygons in Lambert-93 (France): shows CRS \
         handling and reprojection.",
        "polygons_5k_l93.parquet",
    ),
];
        ...
        egui::containers::menu::MenuBar::new().ui(ui, |ui| {
            ui.menu_button("File", |ui| {
                ...
                ui.menu_button(format!("{} Open sample dataset", ph::FLASK), |ui| {
                    ...
                    for (label, hover, file) in SAMPLE_DATASETS {
                        if ui.button(*label).on_hover_text(*hover).clicked() {
                            self.enqueue_load(
                                Source::Remote {
                                    url: format!("{SAMPLE_BASE_URL}/{file}"),
                                    len: 0,
                                },
                                &ctx,
                            );
