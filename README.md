...
- **Catalog browser**: Overture Maps and Geomermaids Parquetry are
  ...
  the map. Open-data portals get their own dialog: paste a portal URL
  and its whole catalog is browsable, for the session or saved for
  good. ArcGIS Hub, Socrata and CKAN all publish one, which is most
  municipal and federal portals in North America.
...
| Open-data portals (`https://data.city.gov/data.json`) | File → Data catalogs…, or paste the catalog URL into File → Open URL… — the portal's datasets are listed with the formats it publishes, searchable by title, keywords and description |
...
Los Angeles, Chicago, Houston and thirteen more — plus twenty state
GIS portals from MassGIS to the California State Geoportal, each
verified to serve openable datasets. A pasted portal lives for the session; once its
...
and imported through the same dialogs a dropped file goes through. The
publisher and licence the catalog states are written beside the import
as an `ATTRIBUTION.txt`, so the layer is credited on the map and stays
credited when the file is reopened months later. Portal catalogs are
fetched fresh each session and never cached: unlike a dated release
prefix, a portal rewrites its catalog in place.
